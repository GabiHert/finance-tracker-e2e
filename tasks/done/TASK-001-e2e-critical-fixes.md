...
await expect(page.getByTestId("goal-alert-banner")).toContainText(
  /exceeded|excedido/i
);
...
const percentText = await goalCard
  .getByTestId("goal-progress-percent")
  .textContent();
...
await expect(
  page.getByTestId("category-name").filter({ hasText: updatedName })
).toBeVisible();

// Verify old name no longer appears
await expect(
  page.getByTestId("category-name").filter({ hasText: originalName })
).not.toBeVisible();
...
if (transactions.length > 0) {
  ...
  const trendLines = trendsChart.locator(
    'path, line, [data-testid="trend-line"]'
  );
...
expect(found).toBeDefined();
expect(found.amount).toBe(150.5);
```
