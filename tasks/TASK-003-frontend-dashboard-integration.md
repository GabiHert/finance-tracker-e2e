...
export async function fetchDashboardSummary(
  period: 'current_month' | 'last_month' | 'last_3_months' | 'year' = 'current_month'
...
export async function fetchCategoryBreakdown(
  ...
  const response = await fetch(`${API_URL}/dashboard/category-breakdown?period=${period}`, {
    headers: getAuthHeaders(),
  })
...
const loadDashboardData = useCallback(async () => {
  try {
    ...
    const breakdown = Object.entries(expensesByCategory).map(([id, amount]) => ({
      id,
