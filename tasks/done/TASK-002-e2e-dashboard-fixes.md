...
const expensesText = await expensesCard
...
const getMetricValues = async () => ({
  income: parseAmount(
    await incomeCard.getByTestId("metric-value").textContent()
  ),
  expenses: parseAmount(
    await expensesCard.getByTestId("metric-value").textContent()
  ),
  balance: parseAmount(
    await balanceCard.getByTestId("metric-value").textContent()
  ),
});
...
page.on("request", (req) => {
  if (
    ...
    req.url().includes("/api/v1/transactions")
  ) {
...
const hasUpArrow = await trendIndicator
  .locator('.text-green-500, [class*="up"], [class*="positive"]')
  .isVisible()
  .catch(() => false);
const hasDownArrow = await trendIndicator
  .locator('.text-red-500, [class*="down"], [class*="negative"]')
  .isVisible()
  .catch(() => false);
...
// Get displayed trend
const trendText = await expensesCard
...
```typescript
// Get initial values
const initialIncome = parseAmount(
  await incomeCard.getByTestId("metric-value").textContent()
);
...
const periodLabel = await page
  .getByTestId("current-period-label")
  .textContent();
...
test("M8-E2E-MOCK: Should NOT display mock data values", async ({ page }) => {
  ...
  const balance = parseAmount(
    await page
      .getByTestId("metric-card-balance")
      .getByTestId("metric-value")
      .textContent()
  );
  const income = parseAmount(
    await page
      .getByTestId("metric-card-income")
      .getByTestId("metric-value")
      .textContent()
  );
  const expenses = Math.abs(
    parseAmount(
      await page
        .getByTestId("metric-card-expenses")
        .getByTestId("metric-value")
        .textContent()
    )
  );
  const savings = parseAmount(
    await page
      .getByTestId("metric-card-savings")
      .getByTestId("metric-value")
      .textContent()
  );
